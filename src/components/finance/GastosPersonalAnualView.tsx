...
const COLOR    = { text: "text-red-500 dark:text-red-400", bg: "bg-red-50 text-red-700 dark:bg-red-950/50 dark:text-red-400" }
...
function QuickAdd({ year, month, categories }: { year: number; month: number; categories: PersonalCategory[] }) {
  ...
  function submit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      await createPersonalTransaction({ year, month, type: "EXPENSE", amount: amt, categoryId: categoryId || undefined })
      setAmount("")
      router.refresh()
      ref.current?.focus()
    })
...
export function GastosPersonalAnualView({ year, monthData, categories }: Props) {
  const router = useRouter()
    ...
    <div className="mx-auto max-w-2xl space-y-6 p-4 md:p-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          <Link href={`/gastos/${year - 1}`} className="flex h-8 w-8 items-center justify-center rounded-lg border border-border text-muted-foreground hover:bg-muted">
          ...
          <div className="rounded-xl border border-border overflow-hidden">
            {categoryTotals.map(({ name, total }) => (
              <div key={name} className="flex items-center gap-3 border-b border-border px-4 py-2.5 last:border-0">
                ...
                <div className="w-24 h-1.5 rounded-full bg-red-100 dark:bg-red-950 overflow-hidden">
                  <div
              ...
              <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
                ...
                <div className="rounded-2xl border border-border bg-card overflow-hidden">
                  <div className="px-4 py-2.5 border-b border-border/50 bg-muted/30">
                    <p className="text-xs font-semibold uppercase tracking-wider text-red-700 dark:text-red-500">Gastos</p>
                  </div>
                  {mTxs.length === 0 && (
